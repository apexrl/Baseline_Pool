...
def simulate_policy(checkpoint_dir,
                    ...
                    **kwargs):
    checkpoint_dir = os.path.join(checkpoint_dir, variant_spec['algorithm_params']['domain'])
    ...
    exp_runner = ExperimentRunner(variant_spec)
...
if __name__ == '__main__':
    ...
    set_seed(args.seed)
    path = simulate_policy(**vars(args))
    print("Example Traj: Reward-{}, Length-{}".format(sum(path[0]['rewards']), len(path[0]['rewards'])))
