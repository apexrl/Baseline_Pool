...
class GaussianPolicy(LatentSpacePolicy):
    def __init__(self,
        ...
        self.shift_and_log_scale_diag_model = self._shift_and_log_scale_diag_net(
            input_shapes=(conditions.shape[1:], ),
            output_size=output_shape[0] * 2,
        )
        
        shift_and_log_scale_diag = self.shift_and_log_scale_diag_model(conditions)
    ...
    def save_weights(self, *args, **kwargs):
        return self.shift_and_log_scale_diag_model.save_weights(*args, **kwargs)

    def load_weights(self, *args, **kwargs):
        return self.shift_and_log_scale_diag_model.load_weights(*args, **kwargs)
